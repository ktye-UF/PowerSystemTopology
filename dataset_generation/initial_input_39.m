function myInput = initial_input_39(dist_type, dist_param)
...
%% load only
load_mean = dist_param.mean;
load_std = dist_param.std;
n_load = dist_param.n_load;
%% Input: load(input_load_bus order), PV(input_pv_bus order)
% % load
% dist_type == 'Gaussian'
for ii = 1 : n_load
    InputOpts.Marginals(ii).Type = dist_type;  % 'Gaussian'
