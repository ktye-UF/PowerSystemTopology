...
addpath('RobustGaSP_matlab/data/humanity_data');
...
model=ppgasp_isotropic(design,response,options);
...
%% Example 3, a simple 1D sin function
...
testing_input=(0:0.01:1)';
testibng_response=sin(testing_input);

pred_model=predict_ppgasp(model,testing_input);

sqrt(mean( (pred_model.mean-testibng_response).^2))
std(testibng_response)
